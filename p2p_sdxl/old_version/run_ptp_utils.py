...
def run_ptp(prompts,image_path=None,inv_mode:Optional[str]=None,use_replace=False,cross_replace_steps :Optional[float] =0.3,
            self_replace_steps:Optional[float] =0.2,seed:Optional[int]=None,blend_word=None,eq_params=None):
    ...
    if inv_mode is not None:
        ...
        (image_gt, image_enc), x_t,x_stars, prompt_embeds,pooled_prompt_embeds = null_inversion.invert(image_path, prompts[0], offsets=(0,0,0,0), verbose=True,train_free=True,all_latents=True)
        ...
        pooled_uncond_embeddings=None
        inversion_guidance=False
    controller = make_controller(prompts, use_replace, cross_replace_steps, self_replace_steps,blend_word,eq_params)
    images, _ = run_and_display(prompts, controller, run_baseline=False, latent=x_t, uncond_embeddings=prompt_embeds,pooled_uncond_embeddings=pooled_prompt_embeds,use_old=False,one_img=False,generator=generator,null_inversion=False,
